//! Fibonacci client with request tracing, latency and throughput measurement.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    socket.set_identity(b"client-1")?;
    ...
    let receiver = thread::spawn(move || {
        ...
        while count < NUM_REQUESTS {
            match channel_clone.recv_timeout(Duration::from_secs(5)) {
                Ok(message) => {
    ...
    let latencies = receiver.join().unwrap_or_default();
